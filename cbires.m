...
%         guidata(hObject, handles);
        ...
        param.fc_prefilt = 4;
        [gist, param] = LMgist(queryImage, '', param);

        nameChange = strrep(name,'_','');
        % construct the queryImage feature vector
        queryImageFeature = [gist str2num(nameChange)];
...
% --- Executes on selection change in popupmenu_HashFunctions.
...
% set variables
if (~isfield(handles, 'HashFunctions') && ~isfield(handles, 'numOfReturnedImages'))
    metric = get(handles.popupmenu_DistanceFunctions, 'Value');
    numOfReturnedImgs = get(handles.popupmenu_NumOfReturnedImages, 'Value');
elseif (~isfield(handles, 'HashFunctions') || ~isfield(handles, 'numOfReturnedImages'))
    if (~isfield(handles, 'HashFunctions'))
        metric = get(handles.popupmenu_DistanceFunctions, 'Value');
        numOfReturnedImgs = handles.numOfReturnedImages;
    else
        metric = handles.HashFunctions;
        numOfReturnedImgs = get(handles.popupmenu_NumOfReturnedImages, 'Value');
    end
else
    metric = handles.HashFunctions;
...
if ( ~isempty( jpg_files ) || ~isempty( png_files ) || ~isempty( bmp_files ) )
    ...
    for k = 1:totalImages
        ...
        nameChange = strrep(name,'_','');
        %construct database
        dataset(k, :) = [gist str2num(nameChange)];
        
        % clear workspace
        clear ('gist','param','image');
    ...
    helpdlg('DB of image features created successfuly!');
